...
function LoginForm() {
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Google-inloggning misslyckades')
  ...
  return (
    <div className="w-full max-w-md">
      <div className="rounded-2xl border bg-white p-8 shadow-sm">
        <h1 className="mb-2 text-2xl font-bold text-slate-900">Välkommen tillbaka</h1>
        <p className="mb-8 text-sm text-slate-500">Logga in på ditt CareerOps-konto</p>

        <button
          onClick={handleGoogleLogin}
          disabled={googleLoading}
          className="mb-6 flex w-full items-center justify-center gap-3 rounded-lg border py-3 font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-50"
        >
          {googleLoading ? (
